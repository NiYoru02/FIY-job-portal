...
echo "Logged in ID: " . ($_SESSION['user_id'] ?? 'NONE');
echo "<br>Role: " . ($_SESSION['role'] ?? 'NONE');
...
if (isset($_POST['post_job'])) {
    // Force the ID to be an Integer to be safe
    ...
    try {
        // We use the exact column names from your database
      $sql = "INSERT INTO jobs (employer_id, job_title, job_description, salary, location, job_type, date_posted) 
        VALUES (:eid, :title, :descr, :sal, :loc, :type, NOW())";
        $stmt = $conn->prepare($sql);
        
        // Execute with a clean array
        $stmt->execute([
            ':eid'   => $employer_id,
            ':title' => $title,
            ':descr' => $desc,
        ...
        // This will tell us EXACTLY what the database hates
